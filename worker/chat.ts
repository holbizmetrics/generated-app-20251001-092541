...
const isGatewayError = (error: unknown): boolean => {
  if (error instanceof OpenAI.BadRequestError) {
    // This checks for the specific error structure from Cloudflare AI Gateway
    const errorBody = error.body as any;
    if (error.status === 400 && errorBody?.code === 2001) {
      return true;
    }
  }
  return false;
};
const GATEWAY_ERROR_MESSAGE = "AI Gateway not configured. Please set up your API keys in the Cloudflare dashboard.";
